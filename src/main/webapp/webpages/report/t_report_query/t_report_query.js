...
function add(){
	clearForm('#editForm');
	$("#editreport_id").attr("disabled",false);
...
function save(){
	$("#editreport_id").attr("disabled",false);
	//必填项没填的
	if($("#editreport_id").val()==""){
		CommonParam.tipsInput("#editreport_id",'请选择报表');
		return false;
	}
	
	if($("#editquery_domains").val()==""){
		CommonParam.tipsInput("#editquery_domains",'请输入所属查询域');
		return false;
	}
	
	if($("#editf_eng").val()==""){
		CommonParam.tipsInput("#editf_eng",'请输入英文名');
		return false;
	}
	if($("#editf_chn").val()==""){
		CommonParam.tipsInput("#editf_chn",'请输入中文名');
		return false;
	}
	if($("#editpriority").val()==""){
		CommonParam.tipsInput("#editpriority",'请输入优先级');
		return false;
	}
	
	//开始提交处理
	var url = 't_report_query/'+WEBOPERATION+'.do';
	var submitData = {};
...
function edit(query_id){
	WEBOPERATION = 'edit';
	var url = 't_report_query/getEditData.do';
	var submitData = {};
	submitData['query_id'] = query_id;
	//一些额外的
	$("#query_id").attr("readonly","readonly");
	$("#query_id").css("color","gray");
	...
	jQueryAjaxForJSON(url, submitData,
			function(sRet) {
				...
				$("#editreport_id").attr("disabled",true);
			}, function(sRet) {
...
//删除
function del(query_id){
	//开始提交处理
	bootbox.confirm("确定要删除该条信息吗?", function(result) {
		if(result) {
			var url = 't_report_query/delete.do';
			var submitData = {};
			submitData['query_id'] = query_id;
			jQueryAjaxForJSON(url, submitData,
					function(sRet) {
