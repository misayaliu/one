...
function save(){
	...
	if($("#editcolumn_name").val()==""){
		CommonParam.tipsInput("#editcolumn_name",'请输入列名称');
		return false;
	}
	...
	//开始提交处理
	var url = 't_report_column/'+WEBOPERATION+'.do';
	var submitData = {};
...
function edit(query_id){
	WEBOPERATION = 'edit';
	var url = 't_report_column/getEditData.do';
	var submitData = {};
	submitData['column_id'] = query_id;
	//一些额外的
...
function del(query_id){
	...
	bootbox.confirm("确定要删除该条信息吗?", function(result) {
		if(result) {
			var url = 't_report_column/delete.do';
			var submitData = {};
			submitData['column_id'] = query_id;
			jQueryAjaxForJSON(url, submitData,
					function(sRet) {
