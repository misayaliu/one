...
function add(){
	...
	$("#chart_query_id").removeAttr("readonly");
	$("#chart_query_id").css("color","black");
...
function save(){
	...
	//开始提交处理
	var url = 't_chart_query/'+WEBOPERATION+'.do';
	var submitData = {};
...
function edit(chart_query_id){
	WEBOPERATION = 'edit';
	var url = 't_chart_query/getEditData.do';
	var submitData = {};
	submitData['chart_query_id'] = chart_query_id;
	//一些额外的
	$("#chart_query_id").attr("readonly","readonly");
	$("#chart_query_id").css("color","gray");
...
//删除
function del(chart_query_id){
	//开始提交处理
	bootbox.confirm("确定要删除该条信息吗?", function(result) {
		if(result) {
			var url = 't_chart_query/delete.do';
			var submitData = {};
			submitData['chart_query_id'] = chart_query_id;
			jQueryAjaxForJSON(url, submitData,
					function(sRet) {
